...
export function LawyerCard({ lawyer, onViewDetails }: LawyerCardProps) {
  const { id, name, avatar, isOnline, rating, yearsOfExperience, specialization, description, tags } = lawyer;
  ...
  return (
    <article className="flex items-center gap-4 px-5 py-4 bg-[#0d1526] border border-[#1B2233] rounded-2xl hover:border-[#2585F4]/30 transition-all duration-200">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            <div className="flex items-center gap-1" aria-label={`Avaliação ${rating.score} de 5 estrelas`}>
              ...
              <span className="ml-1 text-sm font-medium text-white/60">
                {rating.score.toFixed(1)} ({rating.totalReviews})
              </span>
            </div>
          </div>
        </header>

        <p className="text-xs text-white/50 mb-2.5 line-clamp-1">
          {yearsOfExperience} anos de exp. {description}
        </p>
      ...
      <button
        ...
        className="shrink-0 flex items-center justify-center w-10 h-10 bg-[#2585F4] hover:bg-[#1a6fd1] rounded-xl transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#2585F4] focus-visible:ring-offset-2 focus-visible:ring-offset-[#0d1526]"
      >
