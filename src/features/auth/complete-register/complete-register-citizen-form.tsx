...
import {
  completeRegisterCitizenSchema,
  type CompleteRegisterCitizenFormData,
} from "./complete-register-citizen.schema";
...
type FormState = Omit<CompleteRegisterCitizenFormData, never>;
...
export function CompleteRegisterCitizenForm() {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      const role = sessionStorage.getItem("pending_google_role") ?? "citizen";
    ...
    <section className="mx-auto w-full max-w-xl rounded-3xl border border-white/8 bg-[#071735]/80 p-6 shadow-[0_20px_80px_rgba(0,0,0,0.25)] backdrop-blur-sm sm:p-8 lg:mx-0">
      <div className="space-y-2">
        ...
        <p className="text-base text-white/60">
          {userName ? `Olá, ${userName}! ` : ""}Precisamos de mais algumas informações para finalizar sua conta.
        </p>
      </div>
