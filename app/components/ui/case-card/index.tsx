...
const CaseIcon = ({ status }: { status: CaseStatus }) => {
  ...
  if (status === "concluded") {
    return (
      <span className={cn(baseClass, "bg-[var(--status-concluded-bg)]")}>
        <CheckCircleOutlineRounded
          fontSize="small"
          className="text-[var(--status-concluded)]"
          aria-hidden="true"
        />
      </span>
    );
  ...
  return (
    <span className={cn(baseClass, "bg-[var(--status-analysis-bg)]")}>
      <DescriptionRounded
        fontSize="small"
        className="text-[var(--status-analysis)]"
        aria-hidden="true"
      />
    </span>
  );
